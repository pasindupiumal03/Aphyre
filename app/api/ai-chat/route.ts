...
// Helper function to clean excessive markdown formatting
...
const formatMarketDataResponse = async (dataType: string, data: any) => {
  ...
  // First, create a fallback response in case OpenAI is not available
...
- Total Supply: ${ethData?.totalSupply || cgData?.market_data?.total_supply || 'N/A'}
    ...
    const result = await response.json();
    const aiResponse = result.choices?.[0]?.message?.content || 'Unable to generate analysis.';
...
const getGeneralAIResponse = async (message: string, conversationHistory: any[], customSystemMessage?: string) => {
  ...
  // If no OpenAI API key is provided, return helpful crypto-focused response
  if (!OPENAI_API_KEY || OPENAI_API_KEY.includes('your_') || OPENAI_API_KEY.includes('sk-your_')) {
    console.log('OpenAI API key not configured, using fallback response');
    
    // Provide helpful crypto-focused responses without AI
    const lowerMessage = message.toLowerCase();
    ...
    if (lowerMessage.includes('price') && (lowerMessage.includes('bitcoin') || lowerMessage.includes('btc'))) {
      return 'For real-time Bitcoin price data, please ask: "What is the current price of Bitcoin?" and I\'ll fetch live market data for you!';
    }
    
    if (lowerMessage.includes('token') || lowerMessage.includes('address')) {
  ...
  try {
    ...
    return 'I\'m experiencing technical difficulties with advanced AI responses. However, I can still help you with real-time market data! Try asking about Bitcoin price, top gainers, or market trends.';
  }
};
